...
Covers the five named factors (reservoir probability, fault likelihood,
fracture intensity, well distance, confidence), deterministic ordering,
provenance preservation, and byte-valid JSON consumed by the dashboard.
"""
...
class ScoringTests(unittest.TestCase):
    ...
    def test_confidence_bounded_zero_to_one(self) -> None:
        ...
        scores = score_points(analysis)
        for ps in scores:
            self.assertGreaterEqual(ps.confidence_score, 0.0)
            self.assertLessEqual(ps.confidence_score, 1.0)
    ...
    def test_target_ordering_is_deterministic(self) -> None:
        ...
        self.assertEqual(
            [s.point.row for s in first_run],
            [s.point.row for s in second_run],
        )
    ...
    def test_well_near_fault_has_higher_hazard(self) -> None:
        ...
        near_hazards = [s for s in near_scores if s.hazard_score > 0.5]
...
class ExportJsonTests(unittest.TestCase):
    ...
    def test_json_matches_csv_row_schema(self) -> None:
        ...
        with TemporaryDirectory() as export_dir:
            ...
            with result.files["reservoir_targets"].open(encoding="utf-8", newline="") as f:
    ...
    def test_json_hotspot_ordering_descending(self) -> None:
        ...
        with TemporaryDirectory() as export_dir:
            ...
            for i in range(len(hotspots) - 1):
                self.assertGreaterEqual(
                    hotspots[i]["hazard_score"],
                    hotspots[i + 1]["hazard_score"],
                )
