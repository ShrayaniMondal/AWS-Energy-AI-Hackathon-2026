...
**6. Daily Drilling Reports CSV** — `daily_drilling_reports.csv` — 192 rows x 48 columns, one row per rig-day. Wells `PERM-*`, `EF-*`, `BAK-*`, `DL-*` across the Permian, Eagle Ford, and Williston basins. Report dates 2025-02-15 → 2025-02-28. Coverage is uneven by design — 3 to 39 reports per well.
